...
def generate_testset_fid(test_loader, save_path, cuda, dims):
    ...
    for _, (X, y) in enumerate(test_loader):
        ...
        X = X.squeeze()
        save_image(X, os.path.join(path, 'samples', 'raw_images', '{}.png'.format(id)))
        id+=1

    calculate_data_statics(os.path.join(path, 'samples', 'raw_images'),
                           os.path.join(path, 'samples'), 50, True, 2048)
